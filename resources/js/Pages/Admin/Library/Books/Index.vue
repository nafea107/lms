<template>
    <div class="max-w-7xl mx-auto w-full py-8 px-3">
        ...
        <div class="flex justify-between">
            <h3 class="font-rabar-004 text-2xl">{{ $t('الكتب') }}</h3>
        ...
        <Table class="mt-2">
            ...
            <TableBody v-if="!isLoading">
                <TableRow v-for="row in tableData?.data" :key="row.id">
                    <TableCell v-text="row.title[locale]">
                    </TableCell>
                    <TableCell v-text="row.author?.[locale] || '-'">
                    </TableCell>
                    <TableCell v-text="row.library_category?.name[locale]">
                    </TableCell>
                    <TableCell class="flex justify-end ">
                        <div class="flex justify-between gap-1">
                            <Link :href="route('admin.books.edit', {book: row.id})">
                                <Button class="h-[unset] p-2 group border-primary hover:bg-primary" variant="outline">
                                    <Icon class="text-primary group-hover:text-white" icon="tabler:edit"/>
                                </Button>
                            </Link>
...
async function getData() {
    ...
    const res = await axios.get(
        `${route("admin.books.data")}?page=${currentPage.value}&search=${searchTerm.value}`,
    );
...
function removeItem(row) {
    Swal.fire({
        ...
        text: t('عند التاكيد سيتم حذف الكتاب'),
        showCancelButton: true,
        confirmButtonText: t('تاكيد'),
        cancelButtonText: t('الغاء')
    }).then((result) => {
        if (result.isConfirmed) {
            axios.delete(`${route('admin.books.destroy', {book: row.id})}`).then(() => {
                Swal.fire(
                    t('تم الحذف'),
                    t('تم حذف الكتاب بنجاح'),
                    'success'
                );
                getData();
            }).catch((error) => {
