<template>
    <div class="max-w-7xl mx-auto w-full py-8 px-3">
        <Head :title="$t('المراحل')"/>
        <div class="flex justify-between">
            <h3 class="font-rabar-004 text-2xl">{{ $t('المراحل') }}</h3>
            <CreateLevel @get-data="getData"/>
                ...
                <TableRow v-for="row in tableData?.data" :key="row.id">
                    ...
                    <TableCell class="flex justify-end ">
                        <div class="flex justify-between gap-1">
                            <EditLevel :level="row" @get-data="getData"/>
...
import CreateLevel from "@/components/Admin/CreateLevel.vue";
import EditLevel from "@/components/Admin/EditLevel.vue";
...
async function getData() {
    ...
    const res = await axios.get(
        `${route("admin.levels.data")}?page=${currentPage.value}&search=${searchTerm.value}`,
    );
...
function removeItem(row) {
    Swal.fire({
        ...
        text: t('عند التاكيد سيتم حذف المرحلة وكل الكورسات المتعلقة به'),
        showCancelButton: true,
        confirmButtonText: t('تاكيد'),
        cancelButtonText: t('الغاء')
    }).then((result) => {
        if (result.isConfirmed) {
            axios.delete(`${route('admin.levels.destroy', {level: row.id})}`).then(() => {
                Swal.fire(
                    t('تم الحذف'),
                    t('تم حذف المرحلة بنجاح'),
                    'success'
                );
                getData();
            }).catch((error) => {
