<template>
    <div class="max-w-7xl mx-auto w-full py-8 px-3">
        ...
        <div class="flex justify-between">
            <h3 class="font-rabar-004 text-2xl">{{ $t("المواد") }}</h3>
            <CreateSubject @get-data="getData" />
        </div>
        <div class="mt-5">
            <Input
                v-model="searchTerm"
                ...
                <TableRow v-for="row in tableData?.data" :key="row.id">
                    ...
                    <TableCell class="flex justify-end">
                        <div class="flex justify-between gap-1">
                            <EditSubject :subject="row" @get-data="getData" />
                            <Button
...
async function getData() {
    ...
    const res = await axios.get(
        `${route("admin.subjects.data")}?page=${currentPage.value}&search=${
            searchTerm.value
        }`
    );
...
function removeItem(row) {
    Swal.fire({
        ...
        text: t("عند التاكيد سيتم حذف المادة"),
        ...
        if (result.isConfirmed) {
            axios
                .delete(
                    `${route("admin.subjects.destroy", { subject: row.id })}`
                )
                .then(() => {
                    Swal.fire(
                        t("تم الحذف"),
                        t("تم حذف المادة بنجاح"),
                        "success"
                    );
                    getData();
                })
