<template>
    <div class="max-w-7xl mx-auto w-full">
        <h3 class="text-2xl mb-5 font-rabar-004" v-text="$t('اضافة درس')"></h3>
        <Head :title="$t('اضافة درس')"/>
...
const form = useForm({
    ...
    quiz: {
        ...
        question_and_answers: [] // Initialize with deep clone
...
function removeQuestionAndAnswers(index) {
    form.quiz.question_and_answers.splice(index, 1);
}
...
watch(form, (newValue, oldValue) => {
    chapters.value = props.courses.filter(item => item.id.toString() === newValue?.course_id?.toString())[0]?.chapters ?? [];
})
