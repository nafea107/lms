<template>
    <div class="max-w-7xl mx-auto w-full py-8 px-3">
        ...
        <div class="flex justify-between">
            <h3 class="font-rabar-004 text-2xl">{{ $t('الدروس') }}</h3>
        ...
        <Table class="mt-2">
            ...
            <TableBody v-if="!isLoading">
                <TableRow v-for="row in tableData?.data" :key="row.id">
                    ...
                    <TableCell v-text="row.course.name[locale]">
                    </TableCell>

                    <TableCell class="flex justify-end ">
                        <div class="flex justify-between gap-1">
                            <Link :href="route('admin.lessons.edit',{lesson:row.id})">
...
async function getData() {
    ...
    const res = await axios.get(
        `${route("admin.lessons.data")}?page=${currentPage.value}&search=${searchTerm.value}`,
    );
...
function removeItem(row) {
    ...
    Swal.fire({
        ...
        text: t('عند التاكيد سيتم حذف الدرس وكل الاختبارات المتعلقة به'),
        showCancelButton: true,
        confirmButtonText: t('تاكيد'),
        cancelButtonText: t('الغاء')
    }).then((result) => {
        if (result.isConfirmed) {
            axios.delete(`${route('admin.lessons.destroy', {lesson: row.id})}`).then(() => {
                Swal.fire(
                    t('تم الحذف'),
                    t('تم حذف الدرس بنجاح'),
                    'success'
                );
                getData();
            }).catch((error) => {
