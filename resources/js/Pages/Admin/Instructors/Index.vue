<template>
    <div class="max-w-7xl mx-auto w-full py-8 px-3">
        ...
        <div class="flex justify-between">
            <h3 class="font-rabar-004 text-2xl">{{ $t("المعلمين") }}</h3>
            <CreateInstructor @get-data="getData" />
        </div>
        <div class="mt-5">
            <Input
                v-model="searchTerm"
        ...
        <Table class="mt-2">
            ...
            <TableBody v-if="!isLoading">
                <TableRow v-for="row in tableData?.data" :key="row.id">
                    ...
                    <TableCell v-text="row.specialty"></TableCell>
                    <TableCell v-text="row.email"></TableCell>
                    <TableCell>
                        <Badge
                            :variant="row.is_active ? 'success' : 'destructive'"
                        >
                            {{ row.is_active ? $t("نشط") : $t("غير نشط") }}
                        </Badge>
                    </TableCell>
...
import EditInstructor from "@/components/Admin/EditInstructor.vue";
...
async function getData() {
    ...
    const res = await axios.get(
        `${route("admin.instructors.data")}?page=${currentPage.value}&search=${
            searchTerm.value
        }`
    );
...
function removeItem(row) {
    Swal.fire({
        ...
        text: t("عند التاكيد سيتم حذف المدرس"),
